...
t_x1 = tk.texts_to_sequences(test.question1.values.astype(str))
t_x1 = sequence.pad_sequences(t_x1, maxlen=max_len)

t_x2 = tk.texts_to_sequences(test.question2.values.astype(str))
...
SUMBISSIONS_FILE = 'sumbission.gz'
predicted = merged_model.predict([t_x1, t_x2, t_x1, t_x2, t_x1, t_x2], batch_size=384)
output = pd.DataFrame(predicted, columns=['is_duplicate'])
output['test_id'] = np.arange(len(output))
output.to_csv(SUMBISSIONS_FILE, index=False, header=True, columns=['test_id', 'is_duplicate'], compression='gzip')
